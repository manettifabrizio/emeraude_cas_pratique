...
export default class Corn {
	...
	setAssets() {
		...
		this.goodPopCorns = [
			this.resources.items.popcorn_good_1,
			this.resources.items.popcorn_good_2,
			this.resources.items.popcorn_good_3,
		];

		this.badPopCorns = [
			this.resources.items.popcorn_bad_1,
			this.resources.items.popcorn_bad_2,
			this.resources.items.popcorn_bad_3,
		];
	}
	...
	chooseAssets(mesh) {
		...
		const randomSpeed = (Math.random() + 0.2) * 0.004;

		if (randomType < 0.05) {
			mesh.material.map = this.bonusPopCorn;
			mesh.userData.speed = 0.0045;
			mesh.name = "bonus";
